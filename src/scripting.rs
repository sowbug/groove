...
mod nothing {
    ...
    impl ScriptEngine {
        ...
        fn add_sequencer(orchestrator: &mut Orchestrator, device: Rc<RefCell<dyn SequencerTrait>>) {
            orchestrator.add_sequencer(device);
        }
        ...
        fn patch_to_audio_sink(
            upstream: Rc<RefCell<dyn AudioSource>>,
            downstream: Rc<RefCell<dyn AudioSink>>,
        ) {
