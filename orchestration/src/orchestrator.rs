...
mod obsolete {
    ...
    impl Orchestrator {
        ...
        fn add_with_optional_uvid(
            &mut self,
            mut entity: EntityObsolete,
            uvid: Option<&str>,
        ) -> Uid {
            #[cfg(feature = "metrics")]
            ...
            while let Some(command) = commands.pop() {
                ...
                while let Some(message) = messages.pop() {
                    match message {
                        ...
                        GrooveInput::SetSampleRate(sample_rate) => {
        ...
        fn broadcast_midi_messages(
            &mut self,
            channel_message_tuples: &[(MidiChannel, MidiMessage)],
        ) {
        ...
        fn generate_control_update_messages(
            ...
            if let Some(control_links) = self.store.control_links(uid) {
                return control_links.iter().fold(
                    Vec::default(),
                    |mut v, (_target_uid, _param_id)| {
...
pub mod tests {
    ...
    use crate::{entities::EntityObsolete, tests::DEFAULT_BPM};
    use ensnare::prelude::*;
    use groove_core::{
        midi::{MidiChannel, MidiMessage},
    ...
    fn gather_audio_basic() {
        let mut o = Orchestrator::new_with(Clock::default());
    ...
    fn random_access() {
        ...
        let _ = o.add(EntityObsolete::Timer(Box::new(Timer::new_with(
            MusicalTime::new_with_beats(4), // TODO need to look and see what this should be
        ))));
    ...
    fn empty_pattern() {
        ...
        let mut sequencer = Box::new(Sequencer::new_with(&SequencerParams { bpm: DEFAULT_BPM }));
        let mut programmer = PatternProgrammer::new_with(ts);
